<?php
    ...
    $text =$_GET['text'];
    $obj_date =$_GET['obj_date'];
    include_once"../includes/dbh.php";

    if(!empty($id) || !empty($title) || !empty($description) || !empty($text) || !empty($obj_data)){
        $sql ="UPDATE publication SET publication_title='$title', publication_text='$text', publication_description='$description', object_date='$obj_date' WHERE publication_id = '$id';";
        mysqli_query($conn, $sql);

        $sql2 ="DELETE FROM `report` WHERE  = '$id';";
